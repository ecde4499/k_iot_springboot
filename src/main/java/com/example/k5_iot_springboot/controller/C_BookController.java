...
import com.example.k5_iot_springboot.dto.C_Book.BookCreateRequestDo;
...
public class C_BookController {
    ...
    // 1) CREATE
    @PostMapping
    public ResponseEntity<ResponseDto<BookResponseDto>> createBook(@RequestBody BookCreateRequestDo dto) {
    ...
    // @PathVariable
    // : URI 경로의 일부를 변수로 인식해서 값으로 표현
    // - 고정된 리소스 식별자를 전달할 때 사용
    //  => 값이 동수!
    //  => 리소스를 식별하는 역할이므로 RESTful API ㅏ용
    ...
    public ResponseEntity<ResponseDto<List<BookResponseDto>>> getBooksByTitleContaining(
            ...
            // cf) 숫자로 변활할 수 없는 데이터 전달 시 400 Bad Request 발생
    ) {
