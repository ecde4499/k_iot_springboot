...
public class I_ProductController {
    ...
    // 제품 등록
    @PostMapping
    public ResponseEntity<ResponseDto<ProductResponse.Response>> create(
            @AuthenticationPrincipal UserPrincipal userPrincipal,
            @Valid @RequestBody ProductRequest.Create req
    ) {
       ResponseDto<ProductResponse.Response> response = productService.create(userPrincipal, req);
       return ResponseEntity.ok(response);
    }

    // 제품 수정
    @PutMapping(ApiMappingPattern.Products.ID_ONLY)
    public ResponseEntity<ResponseDto<ProductResponse.Response>> update(
            @PathVariable Long productId,
            @AuthenticationPrincipal UserPrincipal userPrincipal,
            @Valid @RequestBody ProductRequest.Update req
    ) {
        ResponseDto<ProductResponse.Response> response = productService.update(productId, userPrincipal, req);
        return ResponseEntity.ok(response);
    }

    // 제품 조회
    @GetMapping(ApiMappingPattern.Products.ID_ONLY)
    public ResponseEntity<ResponseDto<ProductResponse.Response>> getProductById(
            @PathVariable Long productId
    ) {
        ResponseDto<ProductResponse.Response> response = productService.get(productId);
        return ResponseEntity.ok(response);
    }
}
