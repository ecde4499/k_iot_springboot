...
@RequestMapping("/api/v1/students") // 해당 컨트롤러의 공동 URL prefix(접두사) (아래 메서드 경로는 모두 /students로 시작)
...
public class B_StudentController {
    ...
    // 1) 새로운 학생 등록(POST)
    // - 성공 201 Created + location 해더(/students/{id]) + 생성 데이터
    ...
    public ResponseEntity<StudentResponseDto> createStudent(@RequestBody StudentCreateRequestDto requestDto
        ...
        URI location = uriComponentsBuilder // 현재 HTTP 요청의 정보를 바탕으로 설정
                .path("/{id}") // 현재 경로 + /{id}
                .buildAndExpand(created.getId()) // 탬플릿 변수 치환 - 동적 데이터 처리
                .toUri();
    ...
    public ResponseEntity<StudentResponseDto> updateStudent(
            ...
            @RequestBody StudentCreateRequestDto requestDto
    ) {
        StudentResponseDto updated = studentService.updatedStudent(id, requestDto);
