...
public class G_AdminController {
    ...
    public ResponseEntity<ResponseDto<RoleManageResponse.AddRoleResponse>> addRoles(
        ...
        ResponseDto<RoleManageResponse.AddRoleResponse> response = adminService.addRoles(principal, req);
        return ResponseEntity.ok().body(response);
    }

    // 권한 삭제
    @Transactional
    @PostMapping("/roles/remove") // 권한 삭제 != 데이터 삭제 => @PostMapping 사용
    public ResponseEntity<ResponseDto<RoleManageResponse.RemoveRoleResponse>> removeRoles(
        ...
        ResponseDto<RoleManageResponse.RemoveRoleResponse> response = adminService.removeRoles(principal, req);
