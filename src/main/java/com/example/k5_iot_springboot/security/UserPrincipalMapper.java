...
/*
    ...
    => 추후 역할/권한 도입 시 해당 클래스만 변경하면 전영 반영 가능

    cf) Spring Security는 인증/인가 단계에서 UserDetails 인터페이스를 사용
        => UserPrincipal
        - 본 매퍼는 영속 엔티티로부터 인증/인가에 꼭 필요한 값만 뽑아
            , 경랑/불변 VO(UserPrincipal)로 만들어 SecurityContext에 안정하게 전달되도록 하는 매퍼
