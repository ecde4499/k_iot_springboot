...
/*
    ...
    - 엔티티(G_User) 자체를 지니지 않고, 인증에 필요한 값만 안정하게 전달/보관
        => 결합도 낮춤, 캐시/직렬화 안정성 향상

    = 필요성 =
    1) Security의 표준 진입점: AuthenticationProvider는 UserDetails 타입을 통해 사용자 정보와 권한을 검사
        => CustomUserDetailsService#loadUserByUsername() 에서 DB 사용자 정보를 읽고 해당 클래스로 감싼 뒤 반환하면
                , 이후 인증 과정이 표준화되어 동작
    ...
    => 인증 성공 시 Authentication(principal)에 들어가 SecurityContextHolder에 저장됨
        ...
        - JWT 발급 시 플레임으로 id/username/roles를 넣는 출처로 활용
...
public class UserPrincipal implements UserDetails {
    ...
    // === UserDetails 구현 ===
    /*
        Spring Security가 AuthenticationProvider 및 AccessDecisionManager를 통해 인증/인가 수행 시 아래의 메서드 사용
        => 값 반환 이외의 로직 X
     */
