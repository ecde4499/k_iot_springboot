...
/*
    ...
    1. REST란?
        REperesentation State Transfer
        : 자원(Resource)을 URI로 식별하고, HTP 메서드를 이용해 해당 자원에 대한 행위를 정의하는 아키텍처 스타일
        - 자원의 표현: JSON, XML 등
        - 상태 전달: 요청(Request)과 응답(Response)으로 상태 전송
        - HTTP 표준 메서드를 활용

        => 네트워크 ㅏㅇ에ㅓ 클라이언트와 서버 간의 통신 방식 중 하나
    2. RESTful API란?
