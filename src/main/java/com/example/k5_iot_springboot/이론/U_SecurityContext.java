...
/*
    ...
    1. 토큰 생성 (요청 자격 증명)
        # G_AuthServiceImpl
        - new UsernamePasswordAuthenticationToke(req.loginId(), req.password())
    ...
    2. AuthenticationManager.authenticate(...) 호출
        ...
        - 내부에서 여러 AuthenticationProvider 리스트가 있고
            , 토큰 타입을 처리할 수 있는 providers 에게 순서대로 위임

    3. DaoAuthenticationProvider 담당
        - 해당 토큰의 타입(UsernamePasswordAuthenticationToke)을 지원하므로 처리 진입
    ...
    5. 보안 표현으로 매핑
        ...
        - UserPrincipal 생성: id, username, password(해시, 암호화), authorities(ROLE_*), 계정상태 블로그

    6. 비밀번호 검증 (PasswordEncoder)
        - PasswordEncoder.matches(raw, encoded)
            => 불일치: BanCredentialsException
            => 일치 : 다음 단계 진행
    ...
    9. AuthenticationManager가 성공 토큰을 반환
        ...
        - auth.gerPrincipal(): UserPrincipal
        - auth.getAuthorities(): ROLE * 목록

    10. JWT 발급 (애플리케이션 레벨)
        - auth 정보(사용자 PK, username, roles 등)로 AccessToken 생성
        - 클레임 sub=username, roles, iat, exp

    11. 응답 작성
        - 헤더 Authorization: Bearer <토큰> 또는 바디 JSON 변환
        - 외부 클라이언트 요청마다 해당 토큰을 Authorization 헤더로 전송
