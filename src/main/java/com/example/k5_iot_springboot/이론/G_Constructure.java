...
/*
    ...
    1) Controller(Presentation 계층)
        : HTTP 요청을 받음
        - 입력 DTO를 검증(@Valid)하고, 비즈니스 로직은 전주 Service에 위임
    ...
    2) Service(Business 계층)
        ...
        +) DOT <<->> 엔티티 매핑도 발생
        +) createdAt/updatedAt은 앤티티의 @PrePersist/@PreUpdate 대신 서비스 레벨에서 관리

    3) Repository(Persistence 계층)
        : JPA 를 통해 순수한 DB 접근만 담당
        - 쿼리 메ㅓ드, QueryDSL 등을 정의하고 비즈니스 로직은 절대 작성 X

    4) DB
        : 실제 데이터 영속화
        - 인덱스/제약/트랜잭션 수준 등은 무결성과 성능 하양을 담당
 */
