...
/*
    ...
    7. Filter Chain
        - 요청을 차례로 검사하는 필터 묶음
        Ex) 코로나예방 - '교문'(열체크) => '복도'(손소독제) => '문앞'(체온체크)
    ...
    +) 인증 성공: SecurityContext에 저장 -> [Controller(컨트롤러)]
       인증 실패: 401(Unauthorized, 로그인 X -> 인증 X) / 403(Forbidden, 권한 X -> 인증 O but 권한 X)
   ...
   3) AuthenticationManager/Provider - 신분 확인 부서
        : 로그인 시도(/login)가 들어오면, 필터가 아이디/비번을 요청에서 꺼냄
            ...
            => 여러 AuthenticationProvider 에게 일을 시킴
                (Manager 가 Provider 에게)
                - 해당 아이디/비밀번호 일치 확인
                - DB 기반 로그인 소셜 로그인, JWT 검증 등 다양한 Provider가 존재
    4) UserDetailService - 사람 찾기 창구
