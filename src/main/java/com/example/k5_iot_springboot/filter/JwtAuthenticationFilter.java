...
/*
    ...
    : JWT 인증 필터
    - 요청에서 JWT 토큰을 추출
    - request의 header 에서 토큰을 추출하여 검증 (유효한 경우 Security의 context에 인증 정보 저장)
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
     ...
     * => 스프링 시큐리티 필터가 매요청마다 호출하는 핵심 메서드
     *
     * @param request       현재 HTTP 요청 객체
     * @param response      현재 HTTP 요청 응답
     * @param filterChain   다음 필터로 넘기기 위한 체임
    **/
    @Override
    protected void doFilterInternal(
        ...
        try {
            ...
            // 3) 헤더가 없으면(=비로그인 요청) 그냥 통과 - 보호 리소스는 뒤에서 401/403 처라
    ...
    private void setAuthenticationContext (
        ...
        // - 두 번째 인자 Credentials (이미 토큰 검증을 마쳤으므로 mull 전달, 중복 검증 필요 X)
        // - 세 번째 인자 권한 목록
