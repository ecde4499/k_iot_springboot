...
public class JwtProvider {
    ...
    private  final int jwtExpriationMs;
    ...
    public JwtProvider (
        ...
        @Value("${jwt.expriation}") int jwtExpriationMs,
        ...
        this.jwtExpriationMs = jwtExpriationMs;
    ...
    public String generateJwtToken(String username, Set<String> roles) {
        ...
        return Jwts.builder()
                ...
                .setExpiration(new Date(now + jwtExpriationMs)) // 현재 시간에 만료 시간을 더한 설정 (만료 시간)
    ...
    public String removerBearer(String bearerToken) { // 입력: "Bearer <token>"
    ...
    private Claims parseClaimsInternal(String token, boolean allowClockSkewOnExpiry) {
        ...
        try {
            ...
            // 서명 및 기본 도구 검증 후 페이로드(claims)만 추출해 반환
        } catch (ExpiredJwtException ex) {
            // 토쿤이 만료된 경우 발생하는  JJMT 전용 예외 처리
