...
/*
    ...
    cf) JWT
        ...
        +) JWT 구조
            ...
            - payload: 사용자 정보 (예: username-로그인아이디, roles-권한) -> 매개변수로 받아야함
...
/*
    ...
    5) 만료까지 남은 시간 계산                                    - gerRemainingMillis 메서드
 */
...
public class JwtProvider {
    ...
    public JwtProvider (
            // @Value: application.properties나 application.yml과 같은 설정 파일 값을 클래스 변수에 주입
            //      => 데이터 타입 자동 인식
     ...
     * 리프레시(Refresh) 토큰 생성
     * */
    public String generateRefreshKoken(String username, Set<String> roles) {
    ...
    private String buildToken(String username, Set<String> roles, long expirationMs) {
        ...
        return Jwts.builder()
                ...
                .setSubject(username)
                .claim(CLAIM_ROLES, roleList) // 커스텀 클레임 키에 권한 목록 저장 -> setSubject, claim 값은 header에 저장
    ...
    private Claims parseClaimsInternal(String token, boolean allowClockSkewOnExpiry) {
        // allowClockSkewOnExpiry: 완료 직후 허용 오차 적용 여부
        try {
            return parser.parseSignedClaims(token).getPayload();
            // 1) 토큰 서명 검증 (key로 signature 확인)
            // 2) JWT 기본 구조 검사(J.p.s)
            // 3) 성공 시 Claims 꺼내기 가능 (.getPayload() 가능)
        } catch (ExpiredJwtException ex) {
            // 토쿤이 만료된 경우 발생하는  JJWT 전용 예외 처리
            ...
            if (allowClockSkewOnExpiry && clockSkewSeconds > 0 && ex.getClaims() != null) {
                ...
                if (exp != null) {
                    long skewMs = clockSkewSeconds * 1000; // 허용 오차(초)를 밀리초로 변환
                    long now = System.currentTimeMillis();
                    if (now - exp.getTime() <= skewMs) {
                        // 현재 시각 - 만료 시각 <= 허용오차 이내면 "막 만료" 간주
