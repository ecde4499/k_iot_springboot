...
public class WebSecurityConfig {
    ...
    @Value("${cors.allowed-origins:*}") // https://app.example.con, https://admin.example.con
    ...
    public BCryptPasswordEncoder passwordEncoder() {
        ...
        // => 추후 회원가입/로그인 시 passwordEncoder.matches(row, encoded); 비밀번호 비교
    }
    ...
    /*
        ...
        cf) CORS (Cross Origin Resource Sharing)
            : 브라우저(예: 포트번호 - 4178)에서 다른 도메인(Tomcat 서버: 8080)으로부터 리소스를 요청할 때 발생하는 보안 정책
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        source.registerCorsConfiguration("/**", config); // 모든 경로에 동일 CORS 정책 적용 ("/**" = 전체)
        return source;
    }
