...
public enum C_Category {
    // 각 Enum 상수
    // : DB에 저장될 값(dbValue)과 화면에 표시용 값(label)을 함께 정의
    NOVEL("NOVEL", "소설"),
    ESSAY("ESSAY", "에세이"),
    POEM("POEM", "시"),
    ...
    /*
        ...
        : String => Enum
        : JSON 요청 값을 Enum 으로 변환할 때, Enum 이름(name)과 DB 값(dbValue) 모두 인식
        - 대소문ㅁ자 구분 X, 값이 없거나 잘못된 경우 예외 발생
     */
    @JsonCreator
    ...
    public static C_Category fromDbValue(String dbValue) {
        ...
        return Arrays.stream(values())
                .filter(c -> c.name().equals(dbValue))
                .findFirst()
