...
public class D_Post {
    ...
    @JdbcTypeCode(SqlTypes.LONGVARCHAR) // Hibernate 에게 LONGTEXT(LONGVARCHAR)로 기대할 것을 명시
    ...
    @OneToMany(
            ...
            // : Post 내부에서는 읽기 전용 매핑, FK는 D_Comment 테이블이 가짐
    ...
    /*
        ...
        - 해당 설정을 하지 않으면, JPA 의 영속성 컨텍스트가 양방향 관계를 완전히 이해하지 못함!

        cf) 영속성 컨텍스트
            : JPA 에서 엔티티 객체를 연구, 저장하는 환경을 의미
            - 엔티티 매니저에 의해 관리, save(), remove(), find()와 같은 작업을 수행
            - 영속성 컨텍스트에 저장된 엔티티 1차 캐시에 보관
