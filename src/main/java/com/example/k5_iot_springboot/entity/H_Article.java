...
// 매개 변수가 없는 기본 생성자를 Protected 접근 제어자로 생성
// : 외부 클래스에서(엔티티 정의 부분 외) 객체를 직접 생성하는 것을 방지!
// - 무분별한 객체 생성을 억제
// cf) JPA 에서 엔티티 생성 시 기본 생성자가 반드시 필요!
//      => JPA 를 위한 생성자를 프로젝트 전역에서 접근 하는 것을 방지
public class H_Article extends BaseTimeEntity {
    ...
    @Lob
    @JdbcTypeCode(SqlTypes.LONGVARCHAR) // MySQL DB 에서 LONGTEXT와 호환하기 위함
    ...
    // optional = false => 작성자가 무조건 있어야함 = 선택사항 X
    @JoinColumn(
            name = "author_id",
