...
public class F_BoardServiceImpl implements F_BoardService {
    ...
    // == 페이지 네이션 공통: 안전한 Pageable 생성(화이트리스트 정렬) ==
    // : 정렬 키를 그래도 신뢰할 경우, 존재하지 않는 필도 또는 JPA 동적 JPQL 에서 문자열 충돌 발생 가능
    private static final Set<String> ALLOWED_SORTS = Set.of("id", "title", "createdAt", "updatedAt");

    private Pageable bulidPageable(int page, int size, String[] sortParms) {
        // 정렬 파라미터 파싱: ["createdAp,desc", "title,asc] 형태
        Sort sort = Sort.by("createdAt").descending(); // 기본 정렬: 최신순
        // => 정렬 파라미터가 없거나, 전부 화이트리스트에서 무시된 경우 디폴트 정렬을 사용

        if (sortParms != null && sortParms.length > 0) { // 빈 배열이 아닌 경우 (요소 1개 이상)
            // 정렬 순서를 보장할 리스트 - 여러 정렬 기준을 저장 (순서 보장!)
            List<Sort.Order> orders = new ArrayList<>();
            for (String p: sortParms) {
