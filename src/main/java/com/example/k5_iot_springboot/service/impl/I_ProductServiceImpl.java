...
public class I_ProductServiceImpl implements I_ProductService {
    ...
    public ResponseDto<ProductResponse.Response> create(
            UserPrincipal userPrincipal,
            ProductRequest.
            @Valid Create req
    ) {
        ProductResponse.Response data = null;
        ...
        I_Product save = productRepository.save(product);

        data = new ProductResponse.Response(save.getId(), save.getName(), save.getPrice());
    ...
    @Transactional
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseDto<ProductResponse.Response> update(
            Long productId,
            UserPrincipal userPrincipal,
...
//        }
        ...
        data = new ProductResponse.Response(
                product.getId(),
                product.getName(),
                product.getPrice()
        );
    ...
    @Override
    public ResponseDto<ProductResponse.Response> get(Long productId) {
        ProductResponse.Response data = null;

        I_Product product = productRepository.findById(productId)
                .orElseThrow(() -> new EntityNotFoundException("상품을 찾을 수 없습니다."));

        data = new ProductResponse.Response(product.getId(), product.getName(), product.getPrice());
