...
//        req.roles().forEach(user::addRole);
        ...
        RoleManageResponse.UpdateRolesResponse data = new RoleManageResponse.UpdateRolesResponse(
                ...
                // 방어적 복사 - JPA 엔티티 컬렉션 필드를 DTO 내부에서 조작할 경우 캡슐화 깨짐 위험
                user.getUpdatedAt()
        );
    ...
    @Override
    @Transactional
    public ResponseDto<RoleManageResponse.AddRoleResponse> addRoles(UserPrincipal principal, RoleManageRequest.@Valid AddRoleRequest req) {
        G_User user = userRepository.findWithRolesById(req.userId())
    ...
    @Override
    public ResponseDto<RoleManageResponse.RemoveRoleResponse> removeRoles(UserPrincipal principal, RoleManageRequest.@Valid RemoveRoleRequest req) {
        G_User user = userRepository.findWithRolesById(req.userId())
                .orElseThrow(() -> new EntityNotFoundException("해당 ID의 사용자가 없습니다."));
        ...
        // 비워지는 경우 기존 USER 유지(최소 1개 이상의 권한을 가질 것을 보장하는 정책)
