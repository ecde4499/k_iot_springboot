...
//                .gender(req.gender()) // null 허용
    ...
    /**
     * 로그인
     * - 인증 성공 시 Access/Refresh Token 발금
    ...
    public ResponseDto<SignInResponse> signIn(SignInRequest req, HttpServletResponse response) {
        ...
        Authentication auth = authenticationManager.authenticate(
                // 내부에서 DaoAuthenticationProvider가 CustomUserDetailsService.loadUserByUsername(loginId) 호출
                // => G_User 조회
        ...
        String refreshToken = jwtProvider.generateRefreshKoken(req.loginId(), roles);
    ...
    public String refreshAccessToken(String refreshToken) {
        try {
            ...
            // 2) RefreshToken의 subject(=username) 추출
            ...
            if (savedToken.isEmpty() || !savedToken.get().getToken().equals(refreshToken)) {
                throw new IllegalArgumentException("Refresh Token이 없거나 서버에 등록된 것과 일치하지 않습니다.");
            }
