...
public class B_StudentServiceImpl implements B_StudentService {
    ...
    public List<StudentResponseDto> getAllStudents() {
        return studentRepository.findAll()
                .stream()
                .map(this::toDto) // map(student -> toDto(student))
    ...
    @Override
    @Transactional
    public StudentResponseDto updatedStudent(Long id, StudentCreateRequestDto requestDto) {
        B_Student student = studentRepository.findById(id)
                .orElseThrow(() ->
                        new ResponseStatusException(HttpStatus.NOT_FOUND, "해당 id의 학생이 없습니다: " + id));
        ...
        // 2) 필드 변경: 관리 상태인 경우 JPA 는 엔티티의 스냅샷(조회 시점의 원본 값)을 몰래 보관
        //          => 비교 후 Dirty Checking(변경 감지)
        // 3) 데이터 변경 감지 시: UPDATE SQL을 만들어 실행
        // 4) 자동으로 SQL 쿼리 실행

        // cf) CREATE는 새로 만드는 경우(식별자 X) - save 명시 필요!
