...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<ResponseDto<Object>> fail (
        ...
        // 1) reason: 비워질 경우 ErrorCode의 기본 메시지 값 사용
    ...
    private List<FieldErrorItem> toFiledErrors(MethodArgumentNotValidException e) {
        List<FieldErrorItem> list = new ArrayList<>(); // 수집용 리스트
        e.getBindingResult().getFieldErrors().forEach(err -> {
            list.add(new FieldErrorItem(
                    err.getField(),                                                 // 실패한 필드명
                    Objects.toString(err.getRejectedValue(), "null"),   // 거부된 값(null 이면 null)
    ...
    public ResponseEntity<ResponseDto<Object>> handleValidation(MethodArgumentNotValidException e) {
        ...
        return fail(ErrorCode.VALIDATION_ERROR, null, toFiledErrors(e));
    }
    ...
    // === 500 Internal Server Error: 그 밖에 모든 예외에 대한 최종 안전망 === //
    @ExceptionHandler(Exception.class)
    public ResponseEntity<ResponseDto<Object>> handleException(Exception e) {
        log.warn("Internal Error", e);
        return fail(ErrorCode.INTERNAL_SERVER_ERROR, null, null);
    }
}
